...
const CTS = () => {
  ...
  const fetchDoctor = async () => {
    try {
      const res = await AxiosInstance.get(`consultations/${doctorId}/`);
      setDoctor(res.data);
    } catch (err) {
      console.error("Error fetching doctor:", err);
      showSnackbar("Error fetching consultation details", "error");
    }
  };

  const fetchSlots = async () => {
    try {
      const res = await AxiosInstance.get(`ctimeslot/?doctor_id=${doctorId}`);
      setSlots(res.data);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingSlot && editingSlot.id) {
        await AxiosInstance.put(`ctimeslot/${editingSlot.id}/`, payload);
        showSnackbar("Slot updated successfully!");
      } else {
        await AxiosInstance.post("ctimeslot/", payload);
  ...
  const handleDelete = async (slotId) => {
    ...
    try {
      await AxiosInstance.delete(`ctimeslot/${slotId}/`);
      fetchSlots();
    ...
    <Box sx={{ p: 3, bgcolor: "#f9fafc", minHeight: "100vh" }}>
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between", mb: 3 }}>
        <Typography variant="h5" sx={{ fontWeight: 600 }}>
          {doctor ? `${doctor.docname}` : "Consultation"} — Time Slots
        </Typography>
        <Button
          variant="contained"
          startIcon={<AddIcon />}
